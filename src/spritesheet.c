...
static chima_result do_sheet_add_images(chima_sheet_data data,
  ...
  name_len =
    name_len > CHIMA_STRING_MAX_SIZE - 1 ? CHIMA_STRING_MAX_SIZE - 1 : name_len;
  for (chima_size i = 0; i < image_count; ++i) {
    ...
    if (!name) {
      int len =
        sprintf(nodes[i].name.data, "chima_image.%05zu", data->image_count + i);
      nodes[i].name.len =
        len; // sprintf returns the length without null terminator
    } else {
...
chima_result chima_create_spritesheet(chima_context chima,
  ...
  const chima_image** tmp_images =
    CHIMA_MALLOC(chima, total_images * sizeof(const chima_image*));
  if (!tmp_images) {
    return CHIMA_ALLOC_FAILURE;
  }
  ...
  chima_sprite_anim* sprite_anims =
    CHIMA_MALLOC(chima, anim_count * sizeof(chima_sprite_anim));
  if (!sprite_anims) {
  ...
  chima_sprite* sprites =
    CHIMA_MALLOC(chima, total_images * sizeof(chima_sprite));
  if (!sprites) {
    CHIMA_FREE(chima, tmp_images);
  ...
  chima_return ret = chima_create_atlas_image(chima, &sheet->atlas, sprites,
                                              pad, tmp_images, total_images);
...
chima_result chima_load_spritesheet(chima_context chima,
  ...
  chima_sprite* sprites =
    CHIMA_MALLOC(chima, header.sprite_count * sizeof(chima_sprite));
  if (!sprites) {
    CHIMA_FREE(chima, fnames);
    CHIMA_FREE(chima, fanims);
    CHIMA_FREE(chima, fsprites);
  ...
  chima_sprite_anim* anims =
    CHIMA_MALLOC(chima, header.anim_count * sizeof(chima_sprite_anim));
  if (!anims) {
    CHIMA_FREE(chima, sprites);
    CHIMA_FREE(chima, fnames);
    CHIMA_FREE(chima, fanims);
  ...
  if (strncmp(header.image_format, "RAW", 4) == 0) {
    ...
    chima_return ret =
      chima_load_image_mem(chima, &sheet->atlas, chima->atlas_name.data,
...
chima_result chima_write_spritesheet(const chima_spritesheet* sheet,
  ...
  chima_file_sprite* sprites =
    CHIMA_MALLOC(chima, sprite_count * sizeof(chima_file_sprite));
  if (!sprites) {
    return CHIMA_ALLOC_FAILURE;
  }
  ...
  chima_file_anim* anims =
    CHIMA_MALLOC(chima, anim_count * sizeof(chima_file_anim));
  if (!anims) {
    CHIMA_FREE(chima, sprites);
    return CHIMA_ALLOC_FAILURE;
  }
